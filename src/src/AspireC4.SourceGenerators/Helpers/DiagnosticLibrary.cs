...
static class DiagnosticLibrary
{
	...
	public static readonly DiagnosticDescriptor UndeclaredTag = new(
		...
		messageFormat: "Tag '{0}' is not declared. Add it to a 'specification {{ tag {0} }}' block in a .c4 additional file, "
			+ $"or as 'public const string' in the 'Tags' nested class of your {TypeLibrary.LikeC4RegistryAttribute} class.",
		category: "AspireC4",
		...
		description: "All tags passed to WithTag() must be declared in the LikeC4 specification block of an additional "
			...
			+ $"of a {TypeLibrary.LikeC4RegistryAttribute}-annotated class."
	);
	...
	public static readonly DiagnosticDescriptor UndeclaredKind = new(
		...
		messageFormat: "Kind '{0}' is not declared. Add it to a 'specification {{ element {0} }}' or "
			...
			+ $"or as 'public const string' in 'ElementKinds' or 'RelationshipKinds' nested class of your {TypeLibrary.LikeC4RegistryAttribute} class.",
		category: "AspireC4",
		defaultSeverity: DiagnosticSeverity.Warning,
		isEnabledByDefault: true,
		description: "All kinds passed to WithKind() must be declared in the LikeC4 specification block of an additional "
			...
			+ $"RelationshipKinds nested class of a {TypeLibrary.LikeC4RegistryAttribute}-annotated class."
	);
	...
	public static readonly DiagnosticDescriptor MultipleRegistryClassesDefined = new(
		...
		defaultSeverity: DiagnosticSeverity.Warning,
		isEnabledByDefault: true,
		description: $"Only one class per assembly may be annotated with {TypeLibrary.LikeC4RegistryAttribute}. "
	...
	public static readonly DiagnosticDescriptor UndeclaredGroup = new(
		...
		description: "All group names passed to WithLikeC4Group() must be declared as public const string fields "
			+ $"in the Groups nested class of a {TypeLibrary.LikeC4RegistryAttribute}-annotated class."
	);
	...
	public static readonly DiagnosticDescriptor UndeclaredMetadataKey = new(
		...
		messageFormat: $"Metadata key '{{0}}' is not declared. Add it as 'public const string' in the 'MetadataKeys' nested class of your {TypeLibrary.LikeC4RegistryAttribute} class.",
		category: "AspireC4",
		defaultSeverity: DiagnosticSeverity.Warning,
		isEnabledByDefault: true,
		description: "All metadata keys passed to WithMetadata() must be declared as public const string fields "
			+ $"in the MetadataKeys nested class of a {TypeLibrary.LikeC4RegistryAttribute}-annotated class. "
	...
	public static readonly DiagnosticDescriptor DuplicateTypeDeclaration = new(
		...
		messageFormat: $"Registry type '{{0}}' is declared both as a nested class and via {TypeLibrary.KnownTypeAttribute} attributes. Use only one declaration approach per type.",
		category: "AspireC4",
		defaultSeverity: DiagnosticSeverity.Warning,
		isEnabledByDefault: true,
		description: $"A registry type (e.g. Tag, Group) must be declared either as a nested static class "
			+ $"(e.g. 'public static class Tags {{ ... }}') OR via {TypeLibrary.KnownTypeAttribute} attributes on individual constants, not both."
	);
